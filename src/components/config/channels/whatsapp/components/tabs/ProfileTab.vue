...
export default {
  ...
  watch: {
    profileInputs: {
      handler: function (val) {
        ...
        const photoIndex = val.indexOf(
          val.find((input) => input.name === 'profile_image'),
        );
        if (
          val[photoIndex].value[0]?.lastModified !==
          this.profile?.photoFile?.lastModified
        ) {
  ...
  methods: {
    ...
    updateContactInfoInputs(inputData) {
      ...
      if (input?.validator) {
        this.contactInfoInputs[inputData.index].error = !input.validator(
          inputData.value,
        );
      }
      if (inputData.value) {
      ...
      if (this.contactInfo) {
        Object.entries(this.contactInfo).forEach(([key, value]) => {
          if (Array.isArray(value)) {
            ...
            const input = this.contactInfoInputs.find(
    ...
    async isValidPhotoSize(b64ProfilePhoto) {
      const { height, width } =
        await getHeightAndWidthFromDataUrl(b64ProfilePhoto);
    ...
    getProfileInputValue(inputName) {
      return this.profileInputs.find((input) => input.name === inputName)
        ?.value;
    },
    getContactInfoInputValue(inputName) {
      return this.contactInfoInputs.find((input) => input.name === inputName)
        .value;
    },
    handleSave() {
      this.saveProfile();
    ...
    async saveProfile() {
      try {
        ...
        const validSize = b64ProfilePhoto
          ? await this.isValidPhotoSize(b64ProfilePhoto)
          : true;
        if (!validSize) {
          return;
        }
      ...
      for (let key in object) {
        if (Object.prototype.hasOwnProperty.call(object, key)) {
          ...
          if (typeof value === 'object' && value !== null) {
            ...
            if (
              value.length === 0 ||
              value.every((item) => this.isEmpty(item))
            ) {
              delete object[key];
            }
          } else {
