  ...
  <section class="account-verification">
    ...
    <template v-else>
      <UnnnicDisclaimer
        ...
        type="success"
        :title="
          $t('WhatsApp.config.account_verification.disclaimer_verified.title')
        "
        :description="
          $t(
            'WhatsApp.config.account_verification.disclaimer_verified.description',
          )
        "
      />
      ...
      <UnnnicDisclaimer
        ...
        type="neutral"
        :title="
          $t('WhatsApp.config.account_verification.disclaimer_reviewing.title')
        "
        :description="
          $t(
            'WhatsApp.config.account_verification.disclaimer_reviewing.description',
          )
        "
      />
      ...
      <UnnnicDisclaimer
        ...
        :title="
          $t(
            'WhatsApp.config.account_verification.disclaimer_limit_reached.title',
          )
        "
        :description="
          $t(
            'WhatsApp.config.account_verification.disclaimer_limit_reached.description',
          )
        "
      />
      ...
      <UnnnicDisclaimer
        ...
        :description="
          $t('WhatsApp.config.account_verification.disclaimer.description')
        "
      />

      <template v-else>
        <h3>
          {{ $t('WhatsApp.config.account_verification.confirm_section_title') }}
        </h3>
        ...
        <section class="checkboxes">
          <UnnnicCheckbox
            ...
            :textRight="
              $t('WhatsApp.config.account_verification.checkbox_legal_entity')
            "
          />
          <UnnnicCheckbox
            v-model="confirmations.representative"
            :textRight="
              $t('WhatsApp.config.account_verification.checkbox_representative')
            "
          />
          <UnnnicCheckbox
            v-model="confirmations.compliance"
            :textRight="
              $t('WhatsApp.config.account_verification.checkbox_compliance')
            "
          />
        </section>
        ...
        >
          <span class="account-verification__rejection-reasons__label">
            {{
              $t(
                'WhatsApp.config.account_verification.rejection_reasons_label',
              )
            }}:
          </span>
        ...
        <h3 class="mt-6">
          {{ $t('WhatsApp.config.account_verification.upload_section_title') }}
        </h3>
        ...
        <section class="upload-documents">
          <p>
            {{
              $t(
                'WhatsApp.config.account_verification.upload_section_description',
              )
            }}
          </p>
          ...
          >
            <span
              class="upload-documents__file-name"
              :title="file.name"
              >{{ file.name }}</span
            >
          ...
          <UnnnicDisclaimer
            ...
            :title="
              $t(
                'WhatsApp.config.account_verification.suggested_documents.title',
              )
            "
          >
            <template #description>
              <ul>
                <li>
                  {{
                    $t(
                      'WhatsApp.config.account_verification.suggested_documents.item_registration',
                    )
                  }}
                </li>
                <li>
                  {{
                    $t(
                      'WhatsApp.config.account_verification.suggested_documents.item_articles',
                    )
                  }}
                </li>
                <li>
                  {{
                    $t(
                      'WhatsApp.config.account_verification.suggested_documents.item_tax',
                    )
                  }}
                </li>
              </ul>
              ...
              <span>{{
                $t('WhatsApp.config.account_verification.supported_file_types')
              }}</span>
            </template>
          </UnnnicDisclaimer>
...
export default {
  ...
  computed: {
    ...
    hasRejectionReasons() {
      return (
        this.uiState === 'failed' &&
        this.accountVerification?.rejection_reasons?.length > 0
      );
    },
    rejectionReasonsText() {
  ...
  methods: {
    ...
    handleFileInputChange(event) {
      ...
      selectedFiles.forEach((file) => {
        if (!this.isAllowedFile(file)) {
          this.showFileAlert(
            'WhatsApp.config.account_verification.invalid_file_type',
          );
          return;
        }
        ...
        if (file.size > MAX_FILE_SIZE_BYTES) {
          this.showFileAlert(
            'WhatsApp.config.account_verification.file_too_large',
          );
          return;
        }
    ...
    removeFile(index) {
      this.documentFiles = this.documentFiles.filter(
    ...
    async handleSend() {
      ...
      try {
        ...
        unnnic.unnnicCallAlert({
          props: {
            text: this.$t(
              'WhatsApp.config.account_verification.submit_success',
            ),
            type: 'success',
          },
          seconds: 6,
