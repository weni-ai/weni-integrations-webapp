  ...
  <div class="preferences-tab">
    <div class="preferences-tab__scroll">
      <div class="preferences-tab__switches">
        ...
        <section
          v-if="isVersionTwoOrAbove"
          class="preferences-tab__section"
        >
          <h3 class="preferences-tab__section-title">
        ...
        <section class="preferences-tab__section">
          ...
          >
            <UnnnicSelect
              ...
              :placeholder="
                $t('weniWebChat.config.time_between_messages_placeholder')
              "
              @update:model-value="handleTimeBetweenMessagesChange"
            />
          </UnnnicFormElement>
        </section>
      </div>
...
const timeBetweenMessagesOptions = computed(() => [
  {
    label: t('weniWebChat.config.time_between_messages_option', { count: 1 }),
    value: '1',
  },
  {
    label: t('weniWebChat.config.time_between_messages_option', { count: 2 }),
    value: '2',
  },
  {
    label: t('weniWebChat.config.time_between_messages_option', { count: 3 }),
    value: '3',
  },
  {
    label: t('weniWebChat.config.time_between_messages_option', { count: 4 }),
    value: '4',
  },
]);

const timeBetweenMessagesValue = computed(() =>
  String(timeBetweenMessages.value),
);
...
watch(showFullScreenButton, (value) =>
  emit('update:showFullScreenButton', value),
);
watch(startFullScreen, (value) => emit('update:startFullScreen', value));
watch(showVoiceRecordingButton, (value) =>
  emit('update:showVoiceRecordingButton', value),
);
...
watch(useConnectionOptimization, (value) =>
  emit('update:useConnectionOptimization', value),
);
watch(keepHistory, (value) => emit('update:keepHistory', value));
watch(navigateIfSameDomain, (value) =>
  emit('update:navigateIfSameDomain', value),
);
watch(conversationStartersPDP, (value) =>
  emit('update:conversationStartersPDP', value),
);
...
watch(timeBetweenMessages, (value) =>
  emit('update:timeBetweenMessages', value),
);
</script>
