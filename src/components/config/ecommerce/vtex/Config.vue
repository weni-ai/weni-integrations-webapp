<template>
  <div class="config-vtex">
    <div class="config-vtex__header">
      <span
        class="config-vtex__header__description"
        v-html="$t('vtex.config.description')"
      />
    </div>
...
export default {
  ...
  computed: {
    ...
    disableSave() {
      return (
        this.hasConnectedCatalog &&
        this.selectedSellers.length === 0 &&
        this.disableVtexADS
      );
    },
    appConfig() {
      return this.configuredApps?.find((item) => item.uuid === this.appUuid)
        ?.config;
    },
  },
  async mounted() {
  ...
  methods: {
    ...
    async connectCatalog(eventData) {
      ...
      if (this.errorConnectVtexCatalog) {
        this.callModal({
          type: 'error',
          text: i18n.global.t('vtex.errors.connect_catalog'),
        });
        return;
      }
    ...
    async updateConnectedCatalogStatus() {
      ...
      this.hasConnectedCatalog =
        this.currentApp.config?.connected_catalog ?? false;
    },
    ...
    redirectToWppCatalog() {
      if (this.wpp_uuid) {
        this.$router.push({
          path: `/apps/my/wpp-cloud/${this.wpp_uuid}/catalogs`,
        });
      } else {
        this.callModal({
          type: 'error',
    ...
    async handleSave() {
      ...
      this.callModal({
        text: i18n.global.t('vtex.success.sync_sellers'),
        type: 'success',
      });
    },
    updateVtexADS(value) {
