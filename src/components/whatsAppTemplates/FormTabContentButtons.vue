  ...
  <div class="form-tab-content-buttons">
    ...
    <div>
      ...
      <UnnnicSelectSmart
        ...
        @update:model-value="handleButtonTypeChange"
      />
    </div>
    <!-- quick reply -->
    <div
    ...
    >
      ...
      >
        <div class="form-tab-content-buttons__replies__wrapper--inline">
          <span
            class="form-tab-content-buttons__replies__wrapper__button-title"
          >
        ...
        <UnnnicInput
          ...
          :placeholder="
            $t('WhatsApp.templates.form_field.button_text_placeholder')
          "
          :maxlength="25"
          :replaceRegex="EMOJI_REGEX"
          :message="errors[index] || null"
          :type="errors[index] ? 'error' : 'normal'"
          @update:model-value="handleRepliesInput($event, index)"
        />
      </div>
    </div>
    ...
    >
      ...
      >
        <div class="form-tab-content-buttons__call-actions__button__header">
          <span
            class="form-tab-content-buttons__call-actions__button__header__title"
          >
            {{ `${$t('WhatsApp.templates.form_field.button')} ${index + 1}` }}
          </span>
        ...
        <UnnnicSelectSmart
          :class="{
            'form-tab-content-buttons__call-actions__select__disabled':
              disableInputs,
          }"
          ...
          @update:model-value="handleCallToActionTypeChange($event, index)"
        />
        <!-- WEBSITE -->
        <div
          :class="{
        ...
        >
          <UnnnicInput
            ...
            :maxlength="25"
            :replaceRegex="EMOJI_REGEX"
            @update:model-value="handleActionInput($event, 'text', index)"
          />
          <div
          ...
          >
            ...
            >
              ...
              <UnnnicSelectSmart
                ...
                :search="true"
                @update:model-value="handleCountryCodeSelection($event, index)"
              />
            </div>
            ...
            <UnnnicInput
              ...
              :placeholder="
                $t('WhatsApp.templates.form_field.phone_number_placeholder')
              "
              :disabled="disableInputs"
              :modelValue="button.phone_number"
              :maxlength="20"
              @update:model-value="
                handleActionInput($event, 'phone_number', index)
              "
            />
          </div>

          <div
            v-if="button.button_type === 'URL'"
          ...
          >
            ...
            <UnnnicInput
              ...
              :maxlength="2000"
              @update:model-value="handleActionInput($event, 'url', index)"
...
export default {
      ...
      callToActionOptions: [
        {
          value: 'PHONE_NUMBER',
          label: this.$t(
            'WhatsApp.templates.call_to_action_options.call_phone_number',
          ),
        },
        {
          value: 'URL',
          label: this.$t(
            'WhatsApp.templates.call_to_action_options.visit_website',
          ),
        },
      ],
  ...
  mounted() {
    ...
    this.currentButtonType = [
      this.buttonOptions.find((option) => option.value === valueToFind),
    ];
  ...
  computed: {
    ...
    buttonsType() {
      if (!this.templateTranslationCurrentForm.buttons?.length) {
        ...
        this.templateTranslationCurrentForm.buttons[0]?.button_type ===
        'QUICK_REPLY'
      ) {
        return 'quick_reply';
      } else {
    ...
    showAddButton() {
      if (this.buttonsType === 'quick_reply') {
        ...
        return (
          this.currentButtons.length < this.maxActionButtons &&
          this.currentButtons[0].button_type
        );
      } else {
        return false;
      }
  ...
  methods: {
    ...
    handleRepliesInput(event, index) {
      if (countVariables(event) > 0) {
        this.errors[index] = this.$t(
          'WhatsApp.templates.form_field.error_has_variable',
        );
      } else {
    ...
    handleCountryCodeSelection(event, index) {
      ...
      this.buttons[index]['country_calling_code'] = getCountryCallingCode(
        event[0].value,
      );
      this.$emit('input-change', {
        fieldName: 'buttons',
