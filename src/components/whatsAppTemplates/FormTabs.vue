...
export default {
  ...
  beforeUnmount() {
    ...
    if (tabHeader) {
      tabHeader.removeEventListener('wheel', this.listenToWheelEvent, {
        passive: true,
      });
    }
  },
  ...
  methods: {
    ...
    handleWheelEvent(component) {
      component.addEventListener('wheel', this.listenToWheelEvent, {
        passive: true,
      });
    },
    ...
    addTranslation() {
      if (
        this.createdTabs.includes(this.$t('WhatsApp.templates.new_language'))
      ) {
        return;
      }
      this.createdTabs.push(this.$t('WhatsApp.templates.new_language'));
      this.currentTabIndex =
        this.existingTabs.length + this.createdTabs.length - 1;
      this.addNewTranslationForm({
    ...
    handleLanguageChange(event) {
      ...
      this.createdTabs.splice(
        this.currentTabIndex - this.existingTabs.length,
        1,
        event,
      );
    },
    ...
    async createTranslation({ currentTemplateUuid, translationPayload }) {
      ...
      if (!this.errorCreateTemplateTranslation) {
        const createdTabIndex = this.createdTabs.findIndex(
    ...
    buildPayload() {
      try {
        ...
        const fbMessageTemplateId =
          this.templateTranslationCurrentForm?.message_template_id;
    ...
    validateHeader() {
      ...
      if (header?.header_type === 'TEXT') {
        if (header.text) {
          ...
          if (!header.text) {
            errorMsg = this.$t(
              'WhatsApp.templates.error.validation.empty_header',
            );
          } else if (header.text.length >= 60) {
            errorMsg = this.$t(
              'WhatsApp.templates.error.validation.long_header',
            );
          }
        } else {
          header = null;
        ...
        if (!header?.mediaType) {
          errorMsg = this.$t(
            'WhatsApp.templates.error.validation.invalid_media_type',
          );
        } else if (!this.sampleFileData) {
          errorMsg = this.$t(
            'WhatsApp.templates.error.validation.invalid_media_example',
          );
        } else {
    ...
    validateBody() {
      ...
      if (variableCount > 0) {
        if (
          ...
          errorMsg = this.$t(
            'WhatsApp.templates.error.validation.missing_variables_example',
          );
        } else {
          result.example = {
    ...
    validateFooter() {
      ...
      if (!footer) {
        ...
        if (!footer) {
          errorMsg = this.$t(
            'WhatsApp.templates.error.validation.empty_footer',
          );
        }
        if (footer.length >= 60) {
    ...
    validateButtons() {
      ...
      const validButtons = buttons.filter((button) => {
        if (button.text) {
          ...
          if (!button.text) {
            errorMsg = this.$t(
              'WhatsApp.templates.error.validation.empty_button_text',
            );
          }
        } else {
          errorMsg = this.$t(
        ...
        if (button.button_type === 'PHONE_NUMBER') {
          if (button.phone_number) {
            ...
            if (!button.phone_number) {
              errorMsg = this.$t(
                'WhatsApp.templates.error.validation.empty_button_phone_number',
              );
            }
          } else {
            errorMsg = this.$t(
          ...
          if (button.country_code) {
            ...
            if (!button.country_code) {
              errorMsg = this.$t(
                'WhatsApp.templates.error.validation.empty_button_country_code',
              );
            }
          } else {
            errorMsg = this.$t(
        ...
        if (button.button_type === 'URL') {
          if (!button.url) {
            errorMsg = this.$t(
              'WhatsApp.templates.error.validation.empty_button_url',
            );
          } else {
            ...
            if (!this.isValidUrl(buttonURL)) {
              errorMsg = this.$t(
                'WhatsApp.templates.error.validation.invalid_button_url',
              );
            }
          }
        }
