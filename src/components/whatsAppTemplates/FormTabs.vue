<template>
  <div v-if="!loadingFetchWhatsAppTemplate && !loadingWhatsAppTemplates && !dataProcessingLoading" class="form-tabs">
    <unnnic-tab class="form-tabs__tab" v-model="currentTab" :tabs="tabs" :initialTab="initialTranslation"
    ...
    <FormTabContent ref="formContent" class="form-tabs__content" :formMode="currentFormMode" :selectedForm="currentTab"
      :removeLanguages="tabs" :canEdit="canEditTab" :availableLanguages="templateSelectLanguages"
      :loadingSave="loadingSave" @language-change="handleLanguageChange($event)"
      @manual-preview-update="$emit('manual-preview-update')" @save-changes="handleSave" />

    <TranslationSampleForm v-if="showSampleModal" :hasMedia="templateHasMedia()" :hasVariables="templateHasVariables()"
      @close-modal="closeSampleModal" @sample-submission="handleSampleSubmission" />
  </div>
