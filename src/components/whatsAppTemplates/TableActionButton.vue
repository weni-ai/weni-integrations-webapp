<template>
  ...
  >
    ...
    <UnnnicDropdownItem
      ...
      :key="option.id"
      :class="[
        'table-action__dropdown__item',
        `table-action__dropdown__item--${option.id}`,
      ]"
      @click="() => option.action()"
    >
      <UnnnicIconSvg
      ...
      options: [
        ...
        },
        {
          ...
          action: /* istanbul ignore next */ async () => {
            ...
            await this.deleteTemplateMessage({
              appUuid,
            ...
            if (this.errorDeleteTemplateMessage) {
              let errorMsg = this.$t(
                'WhatsApp.templates.error.delete_template',
              );
              const responseError =
                this.errorDeleteTemplateMessage.response.data.error;
              if (responseError) {
