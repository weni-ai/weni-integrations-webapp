...
describe('FormTabContentHeader.vue', () => {
  ...
  it('renders the component correctly', () => {
    ...
    expect(wrapper.findComponent({ name: 'UnnnicSelect' }).exists()).toBe(true);
  });

  it('displays the correct header text when header type is TEXT', () => {
    wrapper = mountComponent();
    const input = wrapper.findComponent({ ref: 'header-text-input' });
  ...
  it('displays text input only when header type is TEXT', () => {
    wrapper = mountComponent();
    expect(wrapper.findComponent({ ref: 'header-text-input' }).exists()).toBe(
      true,
    );
