...
describe('VtexModal.vue', () => {
  ...
  it('renders the second step correctly', async () => {
    process.env.VITE_APP_API_BASE_URL = 'https://integrations-engine.cloud.weni.ai';
    const store = ecommerce_store();
    store.generatedVtexAppUuid = {
      uuid: '1234',
    };
    ...
    expect(webhookUrl).toBe(
      'https://integrations-engine.cloud.weni.ai/api/v1/webhook/vtex/1234/products-update/api/notification/',
    );
  });
